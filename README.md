...
21. Подключение слайдера интервала цен
```
...
08. c-range (слайдер noUiSlider)
09. c-fs (слайдер fslightbox)
10. c-form (форма с кнопкой отправки)
11. c-tabs (табы)
12. c-mode (скрытие части текста)
13. c-checkbox (кастомный чекбокс)
14. c-radio (кастомная радиокнопка)
15. c-tooltip (тултип)
16. c-picture (тег <picture>)
17. c-svgimg (добавление svg-изображения)
18. c-use (спрайты)
19. c-scroll (кнопка вверх сайта)
```

### Список готовых сниппетов CSS, SCSS
...
Некоторые сниппеты тесно связаны с scss-миксинами, например кнопка-бургер. Сниппет __c-burger__ создаст вам html-разметку бургера, а подключение миксина __@include burger__ добавит для него стили, что крайне удобно.
...
Вы можете быстро создать параграф с текстом, который скрываетя и появляется по нажатию на кнопку. Как это использовать:
...
Вы можете быстро создать рабочий noUiSlider-слайдер, для диапазона цен. Как это использовать:

1. В html вызвать сниппет `c-range`. Он создаст базовую структуру слайдера.
2. Раскомментировать строку с подключением стилей в файле _vendor.scss_
3. Подключить слайдер (код в файле _functions.js_).
4. Вызвать миксин `range-slider`

### Слайдер fslightbox

Вы можете быстро создать рабочий fslightbox-слайдер, для галереи фото, видео и тд. Как это использовать:

1. В html вызвать сниппет `c-fs`. Он создаст базовую структуру слайдера.
2. Раскомментировать строку с подключением стилей в файле _vendor.js_
3. Добавьте в `href=` разметки, путь к файлу фото, видео и тд.
